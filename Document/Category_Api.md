...
获取分类列表，支持分页、关键词搜索与隐藏状态筛选。响应中附带可用的隐藏状态筛选项（`filterOptions.hidden`），供前端渲染筛选控件。
...
根据 ID 修改指定分类，可修改分类名称、分类描述、排序顺序、是否隐藏等字段。
...
删除指定 ID 的分类。目前删除操作为**物理删除**：直接调用 `deleteById` 删除记录。
